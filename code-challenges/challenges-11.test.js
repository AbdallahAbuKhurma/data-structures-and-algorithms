...
function transformToLis(obj) {
  ...
  for (let property in obj) {
    keyArray.push(`<li>${property}: ${obj[property]}</li>`);
  }
  return keyArray;
}
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let result = input.map(element => {
    let data = element.filter(number => {
...
let findShortest = (data) => {
  // Solution code here...
  let starArray = [];
  let shortest = data.map (element => {
    let shortestPerson = element.name.match (/([A-z])/g);
    starArray.push(shortestPerson.length);
    return element.name;
  });
  let index = starArray.indexOf (Math.min(...starArray));
