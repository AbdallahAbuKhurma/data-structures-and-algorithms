...
let biggerThanLuke = (arr) => {
  // Solution code here...
  const valuesArray = arr.reduce((acc, value) => {
...
const detectTicTacToeWin = (board) => {
  ...
  for (let i = 0; i < 9; i + 3) {
    if (checkList[i] === checkList[i + 1] && checkList[i] === checkList[i + 2] && checkList[i] !== '') {
      return true;
    } else {
      for (let i = 0; i < 3; i++) {
        if (checkList[i] === checkList[i + 3] && checkList[i] === checkList[i + 6] && checkList[i] !== '') {
          return true;
        } else {
          for (let i = 0; i < 3; i + 2) {
            if (checkList[i] === checkList[4] && checkList[i] === checkList[8 - i] && checkList[i] !== '') {
              return true;
            }
            else {
              return false;
